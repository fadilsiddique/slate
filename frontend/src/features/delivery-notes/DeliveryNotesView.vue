  ...
  <div class="flex flex-col min-h-full">
    ...
    <header class="bg-white sticky top-0 z-10 border-b border-muted/20 shadow-sm">
      ...
      <div class="flex items-center justify-between px-4 pt-4 pb-2.5">
        <div>
          <h1 class="text-xl font-bold text-gray-900 leading-tight">Delivery Notes</h1>
    ...
    <div class="flex-1 px-4 py-3">
      ...
      >
        ...
        <p class="text-gray-700 font-semibold">Couldn't load delivery notes</p>
      ...
      >
        ...
        <p class="text-gray-700 font-semibold">No delivery notes found</p>
        ...
        >
          ...
          <div class="flex items-center justify-between gap-2">
            <span class="text-[11px] text-muted">
              ...
              <span v-if="d.per_billed > 0" class="ml-1 text-green-600/70">
                · Billed {{ d.per_billed }}%
              </span>
            </span>
      ...
      <div v-if="loading && notes.length > 0" class="flex justify-center py-6">
      ...
      <!-- End of results -->
      <p v-if="!hasMore && notes.length > 0 && !loading" class="text-center text-xs text-muted/60 py-5">
        — {{ notes.length }} note{{ notes.length !== 1 ? 's' : '' }} shown —
      </p>
    </div>

    <!-- ── FAB — New Delivery Note ──────────────────────────────────────────── -->
    <button
      ...
      @click="router.push({ name: 'DeliveryNoteNew' })"
    >
...
async function reload(force = false) {
  ...
  try {
    ...
    nextStart.value   = notes.value.length
    totalLoaded.value = notes.value.length
  } catch (e) {
    console.error('[DeliveryNotes] reload failed', e)
    error.value = e?.message ?? 'Failed to load delivery notes'
  } finally {
    loading.value = false
  }
}
...
async function loadMore() {
  ...
  try {
    ...
    console.error('[DeliveryNotes] loadMore failed', e)
  } finally {
    loading.value = false
  }
}
