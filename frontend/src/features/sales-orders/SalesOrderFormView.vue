<template>
  <div class="flex flex-col min-h-full bg-surface">
    ...
    <!-- ── Loading (edit mode / prefill pre-fill) ──────────────────────────── -->
    ...
    <template v-else>
      ...
      <div class="px-4 py-4 pb-44 space-y-5">
        ...
        <!-- ── Dates row 1: Transaction Date + Delivery Date ── -->
        <div class="grid grid-cols-2 gap-3">
          ...
          <div>
            <label class="block text-xs font-semibold text-gray-600 mb-1.5" for="delivery_date">
            ...
            <input
              ...
              type="date"
              class="w-full px-4 py-3 text-sm bg-white rounded-xl border transition focus:outline-none"
              :class="submitted && v.deliveryDate
                ? 'border-red-400 ring-1 ring-red-400'
                : 'border-muted/40 focus:border-primary focus:ring-1 focus:ring-primary'"
            />
            <p v-if="submitted && v.deliveryDate" class="mt-1 text-xs text-red-500">Required</p>
          </div>
        </div>

        <!-- ── Dates row 2: PO Number + PO Date ── -->
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-xs font-semibold text-gray-600 mb-1.5" for="po_no">
              PO Number
            </label>
            <input
              id="po_no"
          ...
          <div>
            <label class="block text-xs font-semibold text-gray-600 mb-1.5" for="po_date">
              PO Date
            </label>
            <input
              id="po_date"
...
// ── Mode detection ────────────────────────────────────────────────────────────
const isEdit    = route.name === 'OrderEdit'
const orderName = isEdit ? route.params.name : null
...
const deliveryDate    = ref(todayPlus30())
const poNo            = ref('')
const poDate          = ref('')
...
const loadingInit = ref(true)   // covers both edit prefill and quotation prefill
...
// ── Page title ────────────────────────────────────────────────────────────────
const pageTitle = computed(() => isEdit ? 'Edit Sales Order' : 'New Sales Order')
...
async function onTaxTemplateChange() {
  ...
  try {
    ...
    const tmpl   = result?.data ?? result
    taxRows.value  = tmpl.taxes ?? []
...
function buildDoc() {
  return {
    ...
    order_type:       'Sales',
    ...(poNo.value.trim() ? { po_no: poNo.value.trim() } : {}),
    ...(poDate.value      ? { po_date: poDate.value }    : {}),
...
async function handleSave() {
  ...
  try {
    const saved = await orderStore.saveOrder(buildDoc())
    router.push({ name: 'OrderDetail', params: { name: saved.name ?? orderName } })
  } catch (e) {
    saveError.value = e?.message ?? 'Failed to save order'
  } finally {
    saving.value = false
  }
}
...
// ── onMounted ─────────────────────────────────────────────────────────────────
onMounted(async () => {
  // Load company defaults and tax templates in parallel
  await Promise.allSettled([
    defaultsStore.fetchDefaults(),
    quotationStore.fetchTaxTemplates(),
  ])

  // Check for prefill from "Create from Quotation" flow
  const prefill = orderStore.consumePrefill()
  if (prefill) {
    customerId.value      = prefill.customer      ?? ''
    customerLabel.value   = prefill.customer_name ?? ''
    customerQuery.value   = prefill.customer_name ?? ''
    transactionDate.value = prefill.transaction_date ?? today()
    deliveryDate.value    = prefill.delivery_date    ?? todayPlus30()
    ...
    if (taxTemplate.value) await onTaxTemplateChange()
  ...
  // Edit mode: pre-fill from existing document
  try {
    const doc = await orderStore.fetchOrderDetail(orderName)
    ...
    deliveryDate.value    = doc.delivery_date    ?? todayPlus30()
    poNo.value            = doc.po_no   ?? ''
    poDate.value          = doc.po_date ?? ''
    ...
    saveError.value = e?.message ?? 'Failed to load order data'
  } finally {
    loadingInit.value = false
  }
})
